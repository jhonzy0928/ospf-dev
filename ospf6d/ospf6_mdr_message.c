			     ...
			     bool *diff, int *hsn)
{
  ...
  *n1 = *n2 = *n3 = *n4 = 0;
  *computing_power = *packet_latency = *packet_loss_rate = *v = *latitude = *longitude = 0;
  ...
  if (hello_tlv)
    {
      ...
      *n3 = hello_tlv->n3;
      *n4 = hello_tlv->n4;
      *computing_power = hello_tlv->computing_power;
      *packet_latency = hello_tlv->packet_latency;
      *packet_loss_rate = hello_tlv->packet_loss_rate;
      *v = hello_tlv->v;
      *latitude = hello_tlv->latitude;
      *longitude = hello_tlv->longitude;
    }
  else
    {
		      ...
		      struct ospf6_lls_header *lls)
{
  ...
  bool twoway = false;
  int n1, n2, n3, n4, n5;
  int computing_power,packet_latency,packet_loss_rate,v,latitude,longitude;
  bool diff;
  int hsn;
  uint32_t *rid;
  ...
  if (ospf6_mdr_process_hello_tlv (on, lls, &n1, &n2, &n3, &n4, &computing_power, &packet_latency, &packet_loss_rate,&v,&latitude,&longitude,&diff, &hsn))
    return;
  //这块是处理邻居接口个数
                            ...
                            bool diff)
{
  ...
  hello_tlv->n3 = n3;
  hello_tlv->n4 = n4;
  hello_tlv->computing_power = computing_power;
  hello_tlv->packet_latency = packet_latency;
  hello_tlv->packet_loss_rate = packet_loss_rate;
  hello_tlv->v = v;
  hello_tlv->latitude = latitude;
  hello_tlv->longitude = longitude;
		      ...
		      void *sendbuf, size_t iobuflen)
{
  ...
  //这块应该改个函数读取
  uint8_t computing_power = 2;
  uint8_t packet_latency = 0;
  uint8_t packet_loss_rate = 2;
  uint8_t v = 2;
  uint8_t latitude = 140;
  uint8_t longitude = 169;
  uint8_t dnl[iobuflen], sanl[iobuflen];
  bool diff = false;
  int size_rid = sizeof (uint32_t);
  ...
  pos += ospf6_mdr_append_hello_tlv (oi, pos, num_lnl, num_hnl,
                                     num_dnl, num_sanl,computing_power,packet_latency,packet_loss_rate,v,latitude,longitude,diff);
